			...
			if (prId != null)
			{
				...
				IReadOnlyList<IssueComment> comments =
					await gitHubClient.Issue.Comment.GetAllForIssue("Testably",
						"Testably.Abstractions.FileSystemGlobbing", prId.Value);
				...
				if (existingComment == null)
				{
					string body = "## :alien: Mutation Results"
					              + Environment.NewLine
					              + $"[![Mutation testing badge](https://img.shields.io/endpoint?style=flat&url=https%3A%2F%2Fbadge-api.stryker-mutator.io%2Fgithub.com%2FTestably%2FTestably.Abstractions.FileSystemGlobbing%2Fpull/{prId}/merge)](https://dashboard.stryker-mutator.io/reports/github.com/Testably/Testably.Abstractions/pull/{prId}/merge)"
					              + Environment.NewLine
					              + string.Join(Environment.NewLine, MutationCommentBodies.Values);

					Log.Information($"Create comment:\n{body}");
					await gitHubClient.Issue.Comment.Create("Testably", "Testably.Abstractions.FileSystemGlobbing",
						prId.Value, body);
				}
				else
				{
					...
					await gitHubClient.Issue.Comment.Update("Testably", "Testably.Abstractions.FileSystemGlobbing",
			...
			{
				{
					...
					[Solution.Tests.Testably_Abstractions_FileSystemGlobbing_Tests]
				}
			};
				...
				string configText = $$"""
				                      {
				                      	"stryker-config": {
				                      		"project-info": {
				                      			"name": "github.com/Testably/Testably.Abstractions.FileSystemGlobbing",
